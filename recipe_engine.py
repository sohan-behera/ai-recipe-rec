...
def _get_secret(key: str) -> str:
    ...
    If not available, use environment/.env.
    """
...
def _passes_filters(
    ...
    if (
        ...
        requested_diet = _normalize(
            diet
        )

        if requested_diet not in tags:
...
4. Do not exceed the available cooking time.
5. Use simple ingredients.
6. Clearly identify the ingredients used.
7. Clearly identify missing ingredients.
8. Give simple step-by-step instructions.
9. Do not invent impossible ingredients.
10. Make recipes suitable for beginners.
