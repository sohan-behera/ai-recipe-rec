...
def verify_recipe(recipe, time_minutes, diet):
    ...
    Validates a recipe generated by Gemini before it is
    ...
    if not isinstance(ingredients, list):
        return False, f"{recipe_name}: ingredients are not in list format."

    if len(ingredients) == 0:
        return False, f"{recipe_name}: ingredients are missing."
    ...
    if not isinstance(steps, list):
        return False, f"{recipe_name}: steps are not in list format."

    if len(steps) == 0:
        return False, f"{recipe_name}: cooking steps are missing."
    ...
    try:
        ...
        return False, f"{recipe_name}: invalid cooking time."

    if estimated_time <= 0:
        return False, f"{recipe_name}: cooking time must be greater than 0."

    if estimated_time > time_minutes:
        return False, (
            f"{recipe_name}: cooking time "
            f"({estimated_time} min) exceeds your "
            f"available time ({time_minutes} min)."
        )

    # -------------------------------------------------------
    # 6. Combine recipe text
    # -------------------------------------------------------
    recipe_text = (
        recipe_name
        + " "
        ...
        for word in non_veg_words:
            if word in recipe_text:
                return False, (
                    ...
                    f"the vegetarian preference."
                )

    # -------------------------------------------------------
    # 8. Vegan verification
    # -------------------------------------------------------
    if diet.lower() == "vegan":
        ...
        for word in non_vegan_words:
            if word in recipe_text:
                return False, (
                    f"{recipe_name}: recipe may not satisfy "
                    f"the vegan preference."
                )

    # -------------------------------------------------------
    # 9. Gluten-free verification
    # -------------------------------------------------------
        ...
        for word in gluten_words:
            if word in recipe_text:
                return False, (
                    ...
                    f"gluten-containing ingredients."
                )
...
with st.sidebar:
    ...
    if GEMINI_READY and TAVILY_READY:
        st.success("⚡ Full AI + Web Mode")
    elif GEMINI_READY:
        st.info("⚡ AI Mode")
    elif TAVILY_READY:
        st.info("🌐 Web Search Mode")
    else:
        st.warning("📦 Local Smart Mode")
        ...
        if not ingredients.strip():
            ...
            if result:
                ...
                if not raw_recipes:
                    ...
                    st.warning(
                        "No recipe was returned by the AI."
                    )
                ...
                # -----------------------------------------------------------
                # ONLY VERIFIED RECIPES CONTINUE
                # -----------------------------------------------------------
                ...
                if recipes:
                    ...
                    st.info(
                        f"🔎 Verify Node completed: "
                        f"{len(recipes)} recipe(s) passed validation."
                    )

                elif raw_recipes:

                    st.error(
                        "❌ No recipes passed the Verify Node. "
                        "Try adding more ingredients or increasing "
                ...
                if use_web_search:
                    ...
                    if not TAVILY_READY:
                        ...
                        st.warning(
                            ...
                            ".env file, so web results can't load. "
...
# ===========================================================================
    ...
    st.write(
        ...
        "(Coming soon — ask me to build this out.)"
    )
